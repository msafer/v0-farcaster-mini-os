...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.fid || !body.username) {
      return NextResponse.json({ error: "Farcaster user data is required" }, { status: 400 })
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: errorData.error || "Authentication failed" }, { status: response.status })
    }
